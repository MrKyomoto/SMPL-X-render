...
# 全局模型和参数（从主模块共享）
body_model = None
shape_params = torch.zeros(1, 10, device=torch.device("cpu"))
pose_params = torch.zeros(1, 156, device=torch.device("cpu"))

# 全局视角变量
current_view_elev = 20
current_view_azim = 45
current_view_dist = 10
...
class AnimationWorker(QThread):
    ...
    def run(self):
        try:
            ...
            for frame_idx in range(total_frames):
                ...
                current_shape = torch.zeros(1, 10, device=torch.device("cpu"))
                ...
                for joint_info in joint_configs:
                    ...
                    if self.interpolation == "smooth":
                        current_rad = smooth_interpolate(
                            ...
                            end_val * np.pi / 180, 
                            t
                        ) if idx != 'global' else smooth_interpolate(
                            start_val * np.pi / 180,
                            end_val * np.pi / 180,
                            t
                        )
                    ...
                    if idx == 'global':
                        ...
                        axis = 1  # 默认值，可替换为 JOINT_AXIS_MAP.get(idx, 0)
    ...
    def _render_frame(self, frame_idx):
        ...
        try:
            ...
            output_file = Path(self.output_path) / f"frame_{frame_idx:04d}.png"
...
# 供外部模块更新全局变量的接口
def update_globals(new_body_model, new_shape_params, new_pose_params, view_elev, view_azim, view_dist):
    global body_model, shape_params, pose_params, current_view_elev, current_view_azim, current_view_dist
    body_model = new_body_model
    shape_params = new_shape_params
    pose_params = new_pose_params
    current_view_elev = view_elev
    current_view_azim = view_azim
    current_view_dist = view_dist
