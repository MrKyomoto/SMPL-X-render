...
class AnimationWorker(QThread):
    ...
    def _render_frame(self, frame_idx):
        ...
        try:
            ...
            if body_model is None:
                ...
                ax.plot_trisurf(vertices[:, 0], vertices[:, 1], vertices[:, 2],triangles=faces, alpha=0.7, color="#4682B4",linewidth=0, antialiased=True)
    ...
    def set_params(self, shape_start, shape_end, joint_configs):
        self._anim_params = {'shape_start': shape_start,'shape_end': shape_end,'joints': joint_configs}
...
start_box = QSpinBox()
        ...
        for name, idx, val in self.core_joints:
            if idx in self.anim_joint_widgets:
                ...
                if checkbox.isChecked():
                    joint_configs.append({'idx': idx,'start_val': start_box.value(),'end_val': end_box.value(),'name': name})
        if len(joint_configs) == 0:
            reply = QMessageBox.question(self, "确认", "没有选择任何关节动画，是否只生成体型动画?",QMessageBox.Yes | QMessageBox.No)
    ...
    def _on_animation_finished(self, output_path):
        ...
        reply = QMessageBox.question(self, "完成",f"动画帧已保存到:\n{output_path}\n是否打开文件夹?",QMessageBox.Yes | QMessageBox.No)
